...
Data is visualized in relation to time. Speed on the Y-axis, Time on the X-axis. The dashboard shows simulated data, generated by running a randomization script on a separate Pi Pico that pushes data in an interval of 30 seconds to 5 minutes. 

Questions to answer by visualizing the data might be:
- How many cars passes in a day? How many of them are speeding?

- When do most of the cars pass by? Is speeding more likely to occur in the morning or in the afternoon?

- Which direction has the most traffic? Do the speeding cars have the same ratio?

- Does the speed of the car change drastically during the measuring event or is it constant? (i.e. does the cat have the intended effect)
...
And it can do everything its big brother can!
![And it can do everything its big brother can](Images/lil_speed_4.png)
Well almost. The greenish cylinders represents 18650 LiPos.